"""
01_bronze_ingest.py — Ingesta Bronze: CSV raw -> Parquet sin transformar

Pipeline:  CSV (pipe-delimited, 110 cols, sin headers)
        -> Renombrar columnas segun glossary
        -> Agregar acquisition_quarter desde nombre del archivo
        -> Parquet particionado por acquisition_quarter

Input:  abfss://raw-csvs@stsynapsemetadata.dfs.core.windows.net/*.csv  (101 archivos, ~820GB)
Output: abfss://synapse-fs@stsynapsemetadata.dfs.core.windows.net/bronze/  (Parquet, ~150-200GB)

Ejecucion: Copiar cada seccion CELDA en una celda separada del notebook Synapse.
           La CELDA 1 (%%configure) DEBE ser la primera celda del notebook.

Tiempo estimado: 13-26h para 820GB (101 archivos).
"""


# ══════════════════════════════════════════════════════════════════════════════
# CELDA 1 — Configuracion Spark (%%configure)
#
...
# }
#
# NOTA: %%configure debe ser la primera celda ejecutable del notebook.
#       El flag -f fuerza reconfiguración incluso si la sesion ya arrancó.
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# CELDA 3 — Schema: 110 columnas, todas StringType
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# CELDA 4 — Listar archivos CSV y verificar estado de reanudacion
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# CELDA 5 — Procesamiento por lotes
#
# Cada lote:
#   1. Lee N archivos CSV con schema explicito de 110 columnas StringType
#   2. Agrega columna acquisition_quarter extraida del nombre del archivo
#   3. Escribe Parquet en modo append, particionado por acquisition_quarter
#
# Si falla a mitad, re-ejecutar desde CELDA 4 para detectar particiones
# ya escritas y continuar desde donde quedo.
# ══════════════════════════════════════════════════════════════════════════════
...
# CELDA 6 — Validacion
#
# Lee el Parquet Bronze completo y verifica:
#   - Total de filas (esperado: ~1-2 billion loan-months)
#   - Total de particiones (esperado: 101, una por trimestre)
#   - Muestra de datos para inspeccion visual
#   - Que acquisition_quarter no tenga valores vacios
# ══════════════════════════════════════════════════════════════════════════════
